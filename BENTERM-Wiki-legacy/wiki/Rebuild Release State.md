...
- Windows toast click-to-focus compiles and now uses the BENTERM AppUserModelID `com.avalonreset.benterm`.
...
- Windows portable packaging exists via `ci/package-benterm-windows.ps1`.
- Local release build succeeded and produced `dist/BENTERM-windows-local-20260421.zip`.
- Installer script is rebranded to BENTERM, uses a unique Inno Setup GUID, installs bundled `wezterm.lua`, fonts, and sounds, and avoids adding `wezterm.exe` to PATH.
- GitHub Actions release workflow exists for tag-driven `benterm-v*` releases across Windows, macOS, and Linux.
- Release tag `benterm-v2026.04.21` is published on GitHub.
- Corrective release work for `benterm-v2026.04.22` removes the old upstream font bundle and keeps only 0xProto plus its license.
...
- Current sound-refresh worktree produced `dist/BENTERM-windows-v2026.04.22-soft-cues-curated.zip` with the re-sourced 84-file cue pack.
