...
- The XML root is `<toast>`. Important attributes for BENTERM are `launch`, `duration`, `scenario`, and `displayTimestamp`.
- `launch` carries activation arguments. BENTERM uses this for `focus-pane:<pane_id>`.
...
BENTERM should avoid `alarm`, `incomingCall`, and `urgent` for normal agent-ready events. `reminder` is the only plausible sticky-toast path. BENTERM uses it for focusable agent-ready toasts with a `Focus` action button plus body click-to-focus, while still clearing the pane's reminder group on user input.
...
- Use a pane-scoped group such as `benterm-agent-ready-pane-<pane_id>`.
...
- A header per pane or window could make BENTERM's Notification Center queue easier to scan.
- Headers are not necessary for the current release because pane groups already give us removal semantics.

## UX Guidance

Microsoft's UX guidance warns that notifications should be useful and not noisy. For BENTERM, that means:
