...
BENTERM is experimenting with a subtle pulsing glow-color shift to terminal text after an agent-ready attention event. This is meant to answer the user-facing question: which terminal is inviting the next prompt?
...
- Enabled by default through `benterm_idle_text_glow`.
- Armed by `start_visual_attention_for_pane`, the same path used by ready sounds and the border/window pulse.
- Starts after `benterm_idle_text_glow_delay_ms`, currently `500`, from the ready event.
- Clears on user input and stays off until the next ready event.
- Suppresses immediately after user input for `benterm_idle_text_glow_input_suppression_ms`, currently `1200`, so a rapid follow-up ready event does not flash instantly.
- Uses `benterm_idle_text_glow_period_ms`, currently `2600`, for a slow pulse.
- Uses `benterm_idle_text_glow_strength`, currently `0.42`, clamped internally.
...
- Built from `E:\benterm-sound-refresh`.
- Current preview executable: `target/debug/BENTERM-gui.exe`, copied from the source-built GUI binary after build.
- Current spawned preview process: `40092`.
- Current preview command is `scripts/preview/launch-benterm-glow-preview.ps1`; it writes `.tmp/benterm-glow-trigger.ps1` and launches PowerShell with `-File` so OSC 777 testing does not depend on fragile inline quoting.
- Branding issue remains for release: the upstream crate still builds `wezterm-gui.exe`; `BENTERM-gui.exe` is currently a post-build preview copy.

Known follow-ups:

- A real release/testing path should include a first-class BENTERM executable name rather than relying on a copied preview binary.
...
- `.tmp/benterm-glow-trigger.ps1` parsed and emitted OSC 777 without PowerShell token errors.
