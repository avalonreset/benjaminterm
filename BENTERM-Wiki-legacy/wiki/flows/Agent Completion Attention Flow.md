...
Goal: when an agent is ready for another prompt, BENTERM should make the correct terminal easy to identify and enter.

Current flow:

1. Agent emits a terminal notification.
2. BENTERM immediately plays the originating pane/tab's attention sound.
3. BENTERM immediately triggers [[Attention Pulse]] for the containing window.
4. BENTERM marks the originating tab through [[Tab Attention Indicator]].
5. The ready event arms [[Idle Text Glow Cue]], which invites the next prompt in the content area.
6. If the originating pane is not already focused, BENTERM shows a Windows toast.
7. User clicks the toast.
8. BENTERM focuses the originating pane/window and triggers visual attention again without replaying the sound.
9. When the user sends the next response into that pane, BENTERM dismisses the pane's outstanding toast and clears the idle glow until the next ready event.

Important rule: the Pavlovian sound cue is not dependent on whether a Windows toast is shown. BENTERM also reports focus-tracking panes as unfocused to terminal applications so agent CLIs keep emitting ready notifications even while the user is actively looking at the pane.
