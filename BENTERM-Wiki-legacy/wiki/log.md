...
- Added `scripts/screenshot-scene.ps1` for printing an up-to-date screenshotable BENTERM terminal scene.
- Added `scripts/launch-screenshot-scene.ps1` to open that scene in BENTERM.
- Linked first README mentions of HyperYap to the HyperYap GitHub repository.
- Converted the approved PNG screenshot to `assets/icon/benterm_screenshot.webp` and pointed the README at it.
...
- Verified `BENTERM-windows-v1.4.0.zip` and `BENTERM-windows-v1.4.1.zip` each contain 84 files under `sounds/benterm-soft-cues` and 0 old `kenney-interface` entries.
- Verified installed `C:/Program Files/BENTERM/sounds/benterm-soft-cues` has 84 WAV files and `C:/Program Files/BENTERM/sounds/kenney-interface` is absent.
...
- Rebuilt `wezterm-gui`, copied it to `target/debug/BENTERM-gui.exe`, stopped only the stale source-preview process, and launched source preview process `40092`.
...
- Rebuilt `wezterm-gui`, copied it to `target/debug/BENTERM-gui.exe`, and launched source preview process `3896`.
...
- Added `scripts/preview/launch-benterm-glow-preview.ps1`, which writes a trigger script and launches it via `-File` to avoid inline quoting failures.
...
- Rebuilt `wezterm-gui`, copied it to `target/debug/BENTERM-gui.exe`, verified BENTERM metadata, and spawned source preview process `33216`.
- Validated with `cargo check -p wezterm-gui --quiet`, `cargo build -p wezterm-gui`, `git diff --check`, and a direct parse run of `.tmp/benterm-glow-trigger.ps1`.

## 2026-04-22 - BENTERM Preview Branding Fix

- User caught that the launched preview was still effectively an upstream-named WezTerm GUI with the wrong app icon.
- Stopped treating copied `wezterm-gui.exe` debug builds as an acceptable BENTERM preview surface.
- Copied the newer BENTERM Windows icon from `E:\benterm\assets\windows\terminal.ico` into this worktree.
- Updated `wezterm-gui/build.rs` Windows resource metadata to ProductName `BENTERM`, OriginalFilename `BENTERM-gui.exe`, and BENTERM file description/company fields.
- Rebuilt `wezterm-gui`, copied the result to `target/debug/BENTERM-gui.exe`, verified version metadata, and spawned process `27216` from that path.

## 2026-04-22 - BENTERM v1.4.0 Finalization

- Built the Windows release binaries with BENTERM Windows resource metadata and refreshed `assets/windows/terminal.ico`.
- Packaged `dist/BENTERM-windows-v1.4.0.zip` with the soft cue set, bundled config, 0xProto fonts, and renamed BENTERM executables.
...
## 2026-04-22 - BENTERM v1.4.0 Installed Locally

- Published GitHub release `v1.4.0` with local Windows portable ZIP, setup EXE, and SHA256 checksum files.
- Initial silent installer return did not replace `C:\Program Files\BENTERM`; verification still showed old WezTerm metadata and the old `kenney-interface` folder.
- Ran an elevated replacement script that stopped installed/source-preview BENTERM processes, removed `sounds/kenney-interface`, and copied `dist/BENTERM-windows-v1.4.0` into `C:\Program Files\BENTERM`.
- Verified installed `BENTERM-gui.exe` hash matches the release package hash `DAA7AAD18288AF5ED99B5938419D69EC673E6803155DEA42F250F649910666D8`.
- Verified installed GUI metadata: ProductName `BENTERM`, FileDescription `BENTERM - agent-focused terminal emulator`, CompanyName `Avalon Reset`, OriginalFilename `BENTERM-gui.exe`.
- Verified installed sound cue set: `sounds/benterm-soft-cues` exists with 84 WAV files, and `sounds/kenney-interface` is absent.
- Updated the pinned taskbar shortcut to target `C:\Program Files\BENTERM\BENTERM-gui.exe` and use icon `C:\Program Files\BENTERM\BENTERM-gui.exe,0`.

## 2026-04-22 - BENTERM v1.4.1 Installer Name Patch

- User caught that the taskbar jump-list displayed `BENTERM` with a space.
- Root cause: the Inno Setup script still defined `MyAppName` as `BENTERM`, so installer-created shortcuts and setup metadata could carry the spaced display name even though the binary metadata was already `BENTERM`.
- Changed the Windows installer AppName and Windows shell verbs to `BENTERM`.
- Added installer cleanup for old spaced Start/Desktop shortcuts and `Open BENTERM here` registry shell verbs.
- Changed the macOS package app directory name from `BENTERM.app` to `BENTERM.app` for consistency.
- Built and published `v1.4.1` with `BENTERM-v1.4.1-setup.exe`, `BENTERM-windows-v1.4.1.zip`, and SHA256 checksum files.
...
- Verified the active public repo is `https://github.com/avalonreset/BENTERM`.
...
- Rebuilt the source GUI and copied it to `target/debug/BENTERM-preview.exe` for a less confusing test surface.
...
- Captured the branding issue that source previews still reference `wezterm-gui.exe`; BENTERM needs a renamed executable or wrapper before release-quality testing.
...
- Rebuilt and relaunched the source preview terminal from `E:\benterm-sound-refresh\target\debug\wezterm-gui.exe` as process `35600`.
...
- Changed the BENTERM design from stable per-pane toast tags to fresh per-event tags plus pane-scoped groups.
...
- Set BENTERM's config layer to suppress toasts from the already-focused pane while still allowing background-tab and other-window toasts.
...
- Made Windows toast XML silent via `<audio silent="true"/>` so the built-in Windows notification sound does not compete with BENTERM's cue.
...
- Removed the runtime fallback path to `kenney-interface` so packaged builds use the new `benterm-soft-cues` directory.
...
- Created `BENTERM-wiki` using the Codex Obsidian LLM Wiki pattern.
