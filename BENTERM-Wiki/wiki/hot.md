...
- Claude Code hooks are spawned with their stdio redirected to pipes AND detached from the parent conpty. A naive `printf '\033]9;...' > /dev/tty` from a hook lands on an ephemeral console BENTERM never sees. The bundled helper walks the parent process chain, `FreeConsole`s, `AttachConsole(parent_pid)`s, and writes OSC 9 to the now-correct `CONOUT$`. Skip `bash.exe` wrappers (they have their own ephemeral consoles); stop on first non-wrapper success to avoid double-firing.
...
- v1.4.4 made BENTERM auto-discover `~/.benterm.lua` before falling back to `~/.wezterm.lua` (and same in `<config-dir>` and `<exe-dir>` on Windows). Plain launches without `--config-file` now load the user's full config.
- `warn_about_missing_glyphs` default is `false` (matching the documented [[Notification Noise Policy]]).
- BENTERM is a cross-platform WezTerm-based terminal distribution. Release target is GitHub release artifacts for Windows, Linux, and macOS.
...
- README positioning: BENTERM as a sensory workflow layer for vibe coding, centered on idle glow cues, pane-aware notifications, soft randomized sounds, fresh theme rotation, and fast attention switching.
...
- `RELEASING_BENTERM.md`, `RELEASE_NOTES_BENTERM.md`, and `README.md` were updated for Mac/Linux release status.
...
- Updated README vision language and mirrored it in [[overview]] and [[BENTERM]].
