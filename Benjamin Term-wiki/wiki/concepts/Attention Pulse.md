...
Attention Pulse is a brief theme-accent edge pulse that appears when an agent-ready notification is received, and again after a toast notification is clicked. It makes the containing terminal window visually obvious when multiple BenjaminTerm windows are visible.

Design constraints:

- Brief, not persistent.
- Drawn by BenjaminTerm, not dependent on Windows taskbar behavior.
