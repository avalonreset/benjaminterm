...
## Pipeline (BenjaminTerm Rust core)
...
- **pwsh / bash plain shell** — bundle a shell-integration script that emits OSC 9 from the `prompt` function on command end. Lives in `assets/shell-integration/benjaminterm.ps1` (pwsh). Sourced from `$PROFILE` or via `default_prog`.
- **Claude Code** — does not emit OSC 9 by default. Bridge it with a `Stop` hook in `~/.claude/settings.json`:
  ...
  "hooks": {
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "printf '\\033]9;BenjaminTerm Ready\\007' > /dev/tty 2>/dev/null || true"
      }]
    }]
  }
  ```
  The hook writes OSC 9 directly to the controlling tty when Claude finishes responding. BenjaminTerm receives it, fires per-pane attention.
