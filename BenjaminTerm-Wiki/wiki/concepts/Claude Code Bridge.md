...
Claude Code does not emit `OSC 9` (the BenjaminTerm agent ready signal per Mandatory Requirements M1) when it finishes a turn. To make Claude trigger the per-pane attention features (border pulse, cursor-row glow, soft-cue sound) the same way Codex does natively, the BenjaminTerm install ships a **Claude Code `Stop` hook helper** at `assets/shell-integration/benjaminterm-claude-stop.ps1`.

## Why a hook helper is needed

A naive Stop hook that just runs `printf '\033]9;BenjaminTerm Ready\007' > /dev/tty` does **not** work on Windows. Two problems:
...
2. **`CONOUT$` opens an ephemeral console.** Claude Code spawns hooks with their console attachment detached from BenjaminTerm's conpty. The hook subprocess gets its own private console nobody reads. `[IO.File]::OpenWrite('CONOUT$')` even fails outright because `CONOUT$` is a device, not a file - it requires `CreateFile` + a `FileStream` constructor that takes the OS handle.
...
4. Stop on first successful write. The first non-wrapper ancestor that
   has a console is the conpty owner BenjaminTerm reads.
```
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "powershell -NoProfile -ExecutionPolicy Bypass -File <install-dir>/shell-integration/benjaminterm-claude-stop.ps1"
          }
        ]
      }
    ]
...
Use forward slashes in the `-File` path. Bash (which Claude uses to run the hook command when `CLAUDE_CODE_GIT_BASH_PATH` is set) treats backslash + letter as an escape and silently mangles the path - `E:\benjaminterm\...` becomes `E:benjaminterm...` and `iscc` errors with "file does not exist."

## Diagnostics

The helper appends every attempt to `%LOCALAPPDATA%\benjaminterm-claude-stop.log` so you can see exactly which ancestor was hit and why. Each Stop hook fire produces one line like:
