...
class OpenInBentermAction(GObject.GObject, Nautilus.MenuProvider):
    def __init__(self):
        super().__init__()
    ...
    def _open_terminal(self, path):
        cmd = ['benterm', 'start', '--cwd', path]
        child = Gio.Subprocess.new(cmd, Gio.SubprocessFlags.NONE)
        if self._systemd:
            # Move new terminal into a dedicated systemd scope so systemd
            # tracks it separately. In particular this keeps a separate
            # CPU and memory account for BENTERM, which prevents oomd from
            # taking nautilus down if a process running inside BENTERM
            # consumes a lot of memory.
            pid = int(child.get_identifier())
            props = [("PIDs", GLib.Variant('au', [pid])),
                ('CollectMode', GLib.Variant('s', 'inactive-or-failed'))]
            name = 'app-nautilus-com.avalonreset.benterm-{}.scope'.format(pid)
    ...
    def _make_item(self, name, paths):
        item = Nautilus.MenuItem(name=name, label='Open in BENTERM',
            icon='com.avalonreset.benterm')
    ...
    def _paths_to_open(self, files):
        ...
        if 10 < len(paths):
            ...
            # to avoid accidentally spamming their desktop with dozens of new
            # windows or tabs. Ten is a totally arbitrary limit :)
            return []
        else:
            return paths
    ...
    def get_file_items(self, *args):
        ...
        if paths:
            return [self._make_item(name='BentermNautilus::open_in_benterm', paths=paths)]
        else:
            return []
    ...
    def get_background_items(self, *args):
        ...
        if paths:
            return [self._make_item(name='BentermNautilus::open_folder_in_benterm', paths=paths)]
        else:
            return []
