# BenjaminTerm Claude Code Stop hook helper.
#
# Emits OSC 9 (the agent ready signal) into BenjaminTerm's pty so the
...
#     console nobody reads, NOT BenjaminTerm's conpty.
#
# Strategy: walk the parent process chain, FreeConsole from the
# ephemeral console, AttachConsole to a parent (Claude Code → pwsh →
# BenjaminTerm), and THEN open CONOUT$. Whichever ancestor is the
# actual conpty owner gives us the pty BenjaminTerm reads.

$logPath = Join-Path $env:LOCALAPPDATA 'benjaminterm-claude-stop.log'
function _Log($msg) {
    try {
...
"@
    ...
    # OSC 9 bytes: ESC ] 9 ; BenjaminTerm Ready BEL
    $bytes = [byte[]]@(
        0x1b, 0x5d, 0x39, 0x3b,
        0x42, 0x65, 0x6e, 0x6a, 0x61, 0x6d, 0x69, 0x6e,
        0x54, 0x65, 0x72, 0x6d, 0x20,
    ...
    # BenjaminTerm's pty, and writing once avoids the double-sound
