...
#
# Emits BEL after each command so the attention pulse, cursor-row glow,
# and soft-cue sound fire on every prompt. Also emits OSC 133 (FinalTerm)
# markers so BenjaminTerm can track prompt boundaries for paste-undo and
# clickable path resolution.
#
# Source from your $PROFILE:
#     . "C:\Program Files\BenjaminTerm\shell-integration\benjaminterm.ps1"
#
# Or set it as part of default_prog in your benjaminterm.lua:
#     config.default_prog = {
...
function global:prompt {
    if ($global:__BTPromptCount -gt 0) {
        ...
        # BEL — fires attention pulse / glow / soft cue in BenjaminTerm
